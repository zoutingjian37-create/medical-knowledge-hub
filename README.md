...
2. 点击“提取并生成预览”。
3. 在“查看与保存”中点击“预览并保存”，检查即将写入的完整提炼结果。
4. 只有在预览弹窗中点击“确认保存到 Obsidian”后才会更新知识库。

原始正文只保存在项目外的临时缓存中，不进入 Git 或 Obsidian。

“默认用 Skill 提炼”开关控制新任务是否自动生成预览；关闭后任务只进入待处理列表，可以随后手动提炼。删除任务会先移入本地回收站，活动列表立即隐藏；回收站可恢复或永久删除，默认保留 7 天，可设置为 1–30 天。永久删除只清理本地任务元数据、临时正文和预览，不会删除 Zotero 文献或已经写入 Obsidian 的内容。
